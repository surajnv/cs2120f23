...
def biimplication : Bool → Bool → Bool
| b1, b2 => and (implies b1 b2) (implies b2 b1)

def eval_bin_op : binary_op → (Bool → Bool → Bool)
| binary_op.and => and
| binary_op.or => or
| binary_op.imp => implies
| binary_op.bimp => biimplication
...
-- #2. (α → Empty ⊕ β → Empty) → (α × β → Empty)
-- In HW 5 Part 1, #2 asked to generalize #1
def e1 := (¬A ∨ ¬B) ⇒ ¬(A ∧ B)

-- #3. no (jam ⊕ cheese) → ((no jam) ⨯ (no cheese))
def e2 := ¬(J ∨ C) ⇒ (¬J ∧ ¬C)

-- #4. ((no jam) ⨯ (no cheese)) → no (jam ⊕ cheese)
def e3 := (¬J ∧ ¬C) ⇒ ¬(J ∨ C)
...
--*IMPLEMENTED ABOVE (lines 58-59)*
/-!
def biimplication : Bool → Bool → Bool
| b1, b2 => and (implies b1 b2) (implies b2 b1)
-/

--*IMPLEMENTED ABOVE (lines 61-65)*
/-!
def eval_bin_op : binary_op → (Bool → Bool → Bool)
| binary_op.and => and
| binary_op.or => or
...
-- (true, true) and (false, false) pairs assessed in #5

-- (true, false) pair
-- α and jam are true
-- β and cheese are false
def true_false : var → Bool
| var.mk 1 => true
| var.mk 3 => true
| _ => false
...
-- (false, true) pair
-- α and jam are false
-- β and cheese are true
def false_true : var → Bool
| var.mk 1 => false
| var.mk 3 => false
| _ => true

-- evaluate (true, false) and (false, true) pairs
#eval eval_expr e0 true_false -- expect true
#eval eval_expr e0 false_true  -- expect true

#eval eval_expr e1 true_false -- expect true
#eval eval_expr e1 false_true  -- expect true

#eval eval_expr e2 true_false -- expect true
#eval eval_expr e2 false_true  -- expect true

#eval eval_expr e3 true_false -- expect true
#eval eval_expr e3 false_true  -- expect true
