...
-- def glue_funs : (β → γ) → (α → β) → α → γ 
-- | g, f, x => g (f x)

-- def glue_funs : (g: β → γ) → (f: α → β) → (a: α) → γ
-- | g,f,a => g (f a)
...
def glue_funs : {α : Type} → {β : Type} → {γ : Type} → (g: β → γ) → (f: α → β) → (a: α) → γ
| _,_,_,g,f,a => g (f a)


-- def glue_funs2 : (α : Type) → (β : Type) → (γ : Type) → (g: β → γ) → (f: α → β) → (a: α) → γ
-- | _,_,_,g,f,a => g (f a)
...
-- #eval glue_funs2 String Nat Bool isEven String.length "Hello"
...
What do you expect the result to be for (glue_funs 
double square 5)? 

I expect the answer to be 50 (2 * (5^2) = 50)

Do you expect to get the same answer for (glue_funs 
square double 5)? In other words is applying double 
after square the sameas applying square after double? 

I don't expect to get the same answer after switching
the order because 2 * (5^2) = 50, but (2 * 5)^2 = 100.
These operations/functions are not commutative.
...
#eval glue_funs double square 4 -- 32
#eval glue_funs square double 4 -- 64
