...
creativity, requiring you to put together several 
...
-- def sumtest1 : Sum Nat Bool := Sum.inr true
-- def sumtest2 := sum_comm sumtest1
...
-- #check sum_comm_reverse sumtest2
-- #reduce sum_comm_reverse sumtest2
...
-- def sumassoctest1 : Sum Nat (Sum String Bool) := Sum.inl 1
-- def sumassoctest2 : Sum Nat (Sum String Bool) := Sum.inr (Sum.inr true)
-- def sumassoctest3 : Sum Nat (Sum String Bool) := Sum.inr (Sum.inl "hi")

-- #reduce sumassoctest1
-- #check sumassoctest2
-- #check sum_assoc sumassoctest2
...
-- #check sumassoctest1
-- #check sum_assoc sumassoctest1
-- #check sum_assoc_reverse (sum_assoc sumassoctest1)
...
-- Here
def its_wet {rain sprinkler : Type} : (rain ⊕ sprinkler) → (rain → wet) → (sprinkler → wet) → wet
...
-- Here:
def sum_elim { α γ : Type } : (α ⊕ γ) → (α → β) → (γ → β) → β 
| (Sum.inl a), a2b, _ => a2b a
| (Sum.inr c), _, c2b => c2b c
