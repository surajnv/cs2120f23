...
def apply {_ : α} : (α → α) → α → α 
| f, a => f a

def apply_twice {_ : α} : (α → α) → α → α 
| f, a => f (f a)
...
#eval map_not [true, true, false, false, true] -- expect [false, false, true, true, false]
...
| [], m => m
| m, [] => m
| e1::l1, e2::l2 =>  e1::(concat l1 (e2::l2))
...
| [] => []
| e::l => concat (reverse l) [e]

#eval reverse [1,2,3,4,5,6] -- expect [6, 5, 4, 3, 2, 1]